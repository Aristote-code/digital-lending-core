...
/**
 * Identity cell. Inside a clickable row this stays plain text — a nested link would
 * steal the click and send people to a different place than the row itself.
 */
...
export function applicationColumns(state: DemoState, extra: Column<Application>[] = []): Column<Application>[] {
  ...
  return [
    { key: "customer", header: "Customer", sortValue: (row) => named(row.customerId), render: (row) => <PersonCell state={state} customerId={row.customerId} subtitle={row.id} /> },
    { key: "stage", header: "Stage", sortValue: (row) => row.stage, render: (row) => row.stage },
    { key: "amount", header: "Requested", align: "right", sortValue: (row) => row.requested, render: (row) => formatRwf(row.requested, true) },
